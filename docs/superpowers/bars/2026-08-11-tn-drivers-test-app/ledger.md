...
| **P3 — Sign system (round 2)** | 2 | **PASS** | A blind critic **broke the round-1 gate with the historical YIELD defect verbatim** (inverted face/legend → `PASS`, exit 0) and with a shape/geometry mismatch. Both are now permanent fixtures. The gate measures face fill at the face's geometric centre (39,222 sample points), parses the outline for shape, and requires ≥90% of the outline to carry paint. **Lead independently reproduced the mutation post-fix: exit 1, correctly named.** Hardening found 3 further real defects: 3 legends fitting at 220px but crossing the face edge at 36px, and `m1-4` publishing a square board as a shield outline (making its shape declaration unfalsifiable). Also fixed `w2-2` (drew the W2-3 figure) and 3 D17 meanings citing the wrong MUTCD section. |
| **P6 — Sign trainer (round 2)** | 2 | **built, not re-judged** | Fixed 4 critic findings. Two were larger than assigned: at 200% **text** size the bottom nav pushed **SIGNS and PROGRESS off a 320px phone** — two destinations unreachable on *every* screen, missed because all reflow tests used page zoom; and the missing space after `·` was the bundled Overpass subset cutting **U+00B7 with a zero advance width**, not a typo. |
| **P7 — Dashboard & onboarding** | 1 | **built, under audit** | 537 tests; X19–X21 proven e2e with **quarantine asserted byte-identical** after recovery. Found a latent bug in P4/P5/P6: `zustand` `persist` runs storage inside `create()`, so their `onRehydrateStorage` notices hit the TDZ and were silently swallowed — **those recovery notices could never have appeared.** Evidence: `evidence/p7-onboarding.png`. |
| **P8 — Progress, settings & rule reference** | 1 | **built, under audit** | 550 tests; charts hand-authored in `src/domain/charts.ts`, nothing carried by colour. Evidence: `evidence/p8-settings.png`. **Lead found a fabricated manual quote** in the settings text-size preview — a sentence appearing nowhere in the manual, rendered in the app's quotation treatment. Replaced with verified p.65 text. The content validator could not have caught it: it audits the bank, and this was a hardcoded component string. |
| **P9 — Offline, PWA & resilience** | 1 | **built, under audit** | Offline proven against the **production** build: load once, go offline, hard-reload, then a study question, a full 30-question exam, the sign library and progress — `net.failed === []`, every response from the service worker, log asserted non-empty so it cannot pass vacuously. **Two thresholds honestly missed and filed, not lowered:** Lighthouse Performance **89** (bar 90; `npm run audit` exits 1) and X23 first-question **2.75s** (bar 2.5s; warm start 387ms). A11y **100**, Best Practices **100**, SEO **100**, installability passes. |
| **P10 — Practices & executable sweep** | — | **in progress** | Auditing all of A1–F6 and X1–X23 independently. |
| **Mobile compatibility** | — | **in progress** | Not in the original decomposition. Added because every test to date ran on a desktop engine with a resized viewport, while the stated design target is an iPhone. Real `webkit`/iPhone 14 and Pixel 7 profiles. |
